# ActivityGraph: local acceptance report

Status: PARTIAL. New evidence from this PC, not the committed release reports.

Repository: https://github.com/danial-maqbool/ActivityGraph
Initial commit: `9e1ece3b657e0bf7ed690d65aa9778a8ad0fab8a`.
Tested source commit: `6f8b2abbbd929bc6be0b8650b2e0019f238b8916`. Source tree was clean for the final source checks; empty uncommitted diff SHA-256: `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`. Subsequent report-only edits are listed in Git.
...
| Full Windows suite | BLOCKED | `verify.py --strict`: 1 | `post-ci-fix-suite/test-report.json` | 135 run, 132 passed, 3 skipped, 0 failures/errors. Strict gate does not pass. |
| Direct-browser checks | PASS | `browser_check.py --slow-api --record`: 0 | `post-ci-fix-browser/` | 20 checks, real loopback backend/downloads. |
...
| Offline runtime | PASS | Network-disabled Docker: 0 | `offline-02/` | Linux container on this PC: 135 run, 135 passed, 0 skipped, 20 browser checks. |
...
| AG-01 | PASS | Commands below and evidence logs | `final-acceptance/results.json` | Discovery/create/edit/rename/delete in selected folder; discovery is not creation. |
| AG-02 | PASS | Commands below and evidence logs | `final-acceptance; post-ci-fix-suite` | Document-content canary absent from event metadata; exclusion and polling bounds. |
| AG-03 | PASS | Commands below and evidence logs | `final-acceptance; post-ci-fix-suite; final-browser` | Notes, tags, search and graph references; real referenced nodes, no measured-hours claim. |
| AG-04 | BLOCKED | Commands below and evidence logs | `post-ci-fix-suite; native desktop not run` | Default-off, consent and exclusion logic checked. Actual title capture, pause/exit and fresh consent after restart need a disposable desktop. |
| AG-05 | PASS | Commands below and evidence logs | `post-ci-fix-suite/verify.log` | Synthetic Chromium/Firefox history only; consent, timestamps, deduplication and URL credential/query/fragment removal. |
| AG-06 | PASS | Commands below and evidence logs | `post-ci-fix-suite/verify.log` | Retention and explicit erase remove history/import keys without deleting monitored files. |
| AG-07 | BLOCKED | Commands below and evidence logs | `final-acceptance; final-worker` | Saved monitoring, process restart, watched event and pause pass. Native install/logout/login/uninstall and OS credential cleanup need disposable session. |
| AG-08 | PASS | Commands below and evidence logs | `final-acceptance; post-ci-fix-suite; offline-02` | Encrypted restart, password/tamper/concurrent-writer checks and offline statistical semantic processing. |
...
The earlier `final-manifest` overlapped the worker-check edit and recorded a mismatch. Verification after the worker fix commit passed; the failure was retained.
...
PASS: [Tests workflow](https://github.com/danial-maqbool/ActivityGraph/actions/runs/34132481972) at `6f8b2abbbd929bc6be0b8650b2e0019f238b8916`. This is remote CI, separate from local acceptance.
